...
#以下为大模型提示词
prompt0=[
        {"role": "system", "content": "你是一个服务于空间粒子探测课题组的科研人员的人工智能，帮助他们进行粒子物理及天文方向科研工作的人工智能助手,你会用精炼的语言准确易懂的回答用户所提提出的各种问题,但不能编造、预测和改编你不知道的信息！不知道就说不知道！忽略prompt中的html标记不要擅自输出！"},
]
prompt=[
        {"role": "system", "content": "你是一个服务于空间粒子探测课题组的科研人员的人工智能，帮助他们进行粒子物理及天文方向科研工作的人工智能助手,你会用精炼的语言准确易懂的回答用户所提提出的各种问题,但不能编造、预测和改编你不知道的信息！不知道就说不知道！忽略prompt中的html标记不要擅自输出！"},
]
...
def createfile(chatbot):
    global prompt
    promptdata = prompt
    # 自定义文件名（示例：时间戳 + 输入前5字符）
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    custom_name = f"DEEPSEEK记忆_{timestamp}.card".replace(" ", "_")
    with open(custom_name, "w", encoding="utf-8") as f:
        json.dump(promptdata, f, ensure_ascii=False, indent=4)  # indent 使格式美观

    return (custom_name)  # 返回元组
...
def read(filename):
    ...
    with open(filename, "r", encoding="utf-8") as f:
        prompt = json.load(f)
        history = convert_prompt_to_history(prompt)
        #print(history)
...
with gr.Blocks(theme=mytheme,title="deepseek调用器",css=mycss) as demo:
    ...
    with gr.Tab('DEEPSEEK-r1'):
        gr.Markdown("## 尊敬的用户您好，欢迎您使用！")
        chatbot = gr.Chatbot()
        msg = gr.Textbox()
        submit_button41 = gr.Button("提交")
        clear_button42 = gr.Button("清除")
        with gr.Row():
            with gr.Column():
                jy1 = gr.File(type="filepath", label="输入记忆卡")
            with gr.Column():
                x1 = gr.Textbox(label="系统提示词（不支持普通用户修改）", value="你是一个服务于空间粒子探测课题组的科研人员的人工智能，帮助他们进行粒子物理及天文方向科研工作的人工智能助手,你会用精炼的语言准确易懂的回答用户所提提出的各种问题,但不能编造、预测和改编你不知道的信息！不知道就说不知道！忽略prompt中的html标记不要擅自输出！")
                submit_button01 = gr.Button("应用记忆（先在左侧上传记忆卡）")
                submit_button02 = gr.DownloadButton("下载保存本次记忆")

        msg.submit(user, [msg, chatbot], [msg, chatbot], queue=False).then(deepseekr1, chatbot, chatbot)
        submit_button01.click(read, jy1, [msg, chatbot], queue=False)
        submit_button02.click(fn=createfile, inputs=chatbot, outputs=submit_button02)
        submit_button41.click(user, [msg, chatbot], [msg, chatbot], queue=False).then(deepseekr1, chatbot, chatbot)
        clear_button42.click(tag, None, chatbot, queue=False)
    ...
    gr.Markdown("> 作者信息：刘浩然 物理学院B8大厅 有事线下找我")
...
# 启动 Gradio 应用的线程
g_thread = threading.Thread(target=launch_g, args=(demo,))
g_thread.start()